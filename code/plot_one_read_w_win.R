...
#  id is the readID. it must contain only one value as we are plotting
#  only one read.
#  start and end are positions on the reference genome.
#  val is the probability of the modification for raw or windowed data.
#  label is the the type of data. allowed labels are 'rawVal' for raw data
#  and 'winVal' for windowed data.
...
if (length(unique(read_data$id)) > 1) {
  stop("Id column cannot have multiple values", call. = FALSE)
}
...
plot1 <-  ggplot() +
      geom_point(data = subset(read_data, label == "rawVal"),
        aes(x = start, y = val, colour = label), shape = "circle",
            alpha = 0.2, show.legend = TRUE) +
      geom_segment(data = subset(read_data, label == "winVal"),
        aes(x = start, y = val, xend = end, yend = val,
            colour = label), size = 2, show.legend = FALSE) +
